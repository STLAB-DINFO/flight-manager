...
//--------- questo sotto è da aggiungere per la soluzione corretta
...
//  --------------- private methods -----------------------
	...
	private void saveFlight() {
		...
		if (insertFlightBackToo) {
			...
			backFlight.copyFlight(flight);
			backFlight.setSourceAirport(flight.getDestinationAirport());
			backFlight.setDestinationAirport(flight.getSourceAirport());
		...
		for (int i = 0; i < repeats.intValue() - 1; i++) {
			...
			flightDao.save(fl);
			if (insertFlightBackToo) {
				Flight backfl = ModelFactory.flight();
				backfl.copyFlight(flight);
				backfl.setSourceAirport(flight.getDestinationAirport());
				backfl.setDestinationAirport(flight.getSourceAirport());
				backfl.setDepartureDate(date);
				flightDao.save(backfl);
			}
		}
	}
